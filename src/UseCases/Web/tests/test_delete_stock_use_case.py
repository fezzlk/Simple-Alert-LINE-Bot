...
def test_delete_stock_missing_id():
    repo = DummyStockRepository(update_result=1)
    use_case = DeleteStockUseCase(stock_repository=repo)
...
def test_delete_stock_success_updates_status():
    repo = DummyStockRepository(update_result=1)
    use_case = DeleteStockUseCase(stock_repository=repo)
    ...
    assert isinstance(repo.last_query["_id"], ObjectId)
    assert repo.last_values == {"status": 2}
