...
class HabitTaskLogRepository:
    _collection_name = "habit_task_logs"
    ...
    def create(self, new_log: HabitTaskLog) -> HabitTaskLog:
        data = new_log.__dict__.copy()
        if data["_id"] is None:
            data.pop("_id")
    ...
    def find(
        ...
        for record in records:
            ...
            data["_id"] = record.id
            items.append(HabitTaskLog(**data))

        items.sort(key=lambda i: (i.scheduled_date or "", i._id or ""))
        if sort is None:
            return items
